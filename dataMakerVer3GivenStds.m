...
% list of factor volatilities, and a seed value for replication, the
% function generates simulated market data.  The method outputs a matrix of
% market returns of size nDays x nMkts, a myBetas matrix containing each
% market's betas to each true factor (the matrix is nMkts x nTrueFactors),
% the true factor returns matrix which is nDays x nTrueFactors, and the
% matrix of idiosyncratic returns which is nDays x nMkts. 'Drift' specifies
% the amount of drift of the first factor (essentially the mean of the
% returns). The idioVolPercent specifies the amount of each market's
% volatility that is purely idiosyncratic (meaning orthogonal to all
% factors and all other markets' idiosyncratic components). Setting this
% parameter to 0.01 gives each market 1% idiosyncratic volatility. The
% myFactorStds parameter is a list of the volatilities of each factor. The
% seedVal paramter allows the user to specify the seed to be used for
% random number replication. 
...
function [mktRtns, myBetas, factorRtns, idioRtns] = dataMakerVer3GivenStds(nDays, nMkts, drift, idioVolPercent, myFactorStds, seedVal)
    format long 
