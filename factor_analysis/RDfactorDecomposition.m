function [estFactorRtns, portBetas, factorVols] = RDfactorDecomposition(mktRtns, myPositions, params)
    %% factorDecomposition
    ...
    % Access tickers from params
    if isfield(params, 'tickers')
    ...
    variableVariances = var(mktRtnsLookbackAdj);
    zeroVarianceColumns = variableVariances < 1e-10;
    if any(zeroVarianceColumns)
        disp(['Removing ', num2str(sum(zeroVarianceColumns)), ' variables with zero or near-zero variance.']);
        mktRtnsLookbackAdj(:, zeroVarianceColumns) = [];
        mktRtns(:, zeroVarianceColumns) = [];
        myPositions(zeroVarianceColumns) = [];
        if ~isempty(tickers)
            tickers(zeroVarianceColumns) = [];
        end
    end

    % Recompute nMkts after removing variables
    [~, nMktsAdjusted] = size(mktRtnsLookbackAdj);
    disp(['Number of markets after removing variables: ', num2str(nMktsAdjusted)]);

    % Computing correlation matrix
    corrMatrix = corr(mktRtnsLookbackAdj);
    
    % Setting up Eigen Value Decomposition object
    evd = RDeigenValueDecomposition;
    evd.corrMatrix = corrMatrix;

    % Performing eigenvalue decomposition based on model type
    ...
    elseif params.modelType == "PAF"
        ...
        % Principal Axis Factoring (PAF)
        [eigenValues, eigenVectors, communalities] = evd.PAF(params.tolerance, params.iterations);
    ...
    plotVis = RDvisualization;
    plotVis.factorLoadings = factorLoadings;
    plotVis.eigenValues = eigenValues;
    if exist('communalities', 'var')
        plotVis.communalities = communalities;
    end
    if ~isempty(tickers)
        plotVis.tickers = tickers;
    end

    % Visualize eigenvalues & communalities if needed
    if params.visualize
        plotVis.plotEigenValues();
        if exist('communalities', 'var')
            plotVis.plotCommunalities();
        end
    end

    % Setting up factor rotation object
    rotation = RDfactorRotation;
    ...
    if ~isempty(params.rotationType)
        rotation = factorRotation;
        rotation.factorLoadings = factorLoadings;

        switch params.rotationType
            case 'varimax'
                factorLoadings = rotation.varimaxRotation(params.builtInNormalizeLoadings);
            case 'quartimax'
                factorLoadings = rotation.quartimaxRotation(params.builtInNormalizeLoadings);
            case 'equamax'
                factorLoadings = rotation.equamaxRotation(params.builtInNormalizeLoadings);
            case 'promax'
                factorLoadings = rotation.promaxRotation(params.builtInNormalizeLoadings);
            case 'orthomax'
                factorLoadings = rotation.orthomaxRotation(params.builtInNormalizeLoadings, params.orthoGamma);
            case 'parsimax'
                factorLoadings = rotation.parsimaxRotation(params.builtInNormalizeLoadings);
            case ""
            otherwise
                error( ...
                    ['Invalid rotationType: %s\nValid types: ' ...
                    'varimax quartimax, equamax, promax, orthomax, parsimax'], ...
                    params.rotationType);
        end
        % Update factorLoadings in visualization object
        plotVis.factorLoadings = factorLoadings;
    end

    % Visualize factor loadings before or after rotation
    if params.visualize == true
        if ismember(params.visualizeBeforeAfterRotation, {'before', 'both'})
            % Visualize before rotation
            plotVis.factorLoadings = eigenVectors(:, 1:k);  % Before rotation
            plotVis.visualizeFactorLoadingsHeat(k, params.rotationType, params.numVariablesToShow, 'Before Rotation');
            plotVis.visualizeFactorLoadingsBar(k, params.rotationType, params.numVariablesToShow, 'Before Rotation');
        end
    
        if ismember(params.visualizeBeforeAfterRotation, {'after', 'both'})
            % Visualize after rotation
            plotVis.factorLoadings = factorLoadings;  % After rotation
            plotVis.visualizeFactorLoadingsHeat(k, params.rotationType, params.numVariablesToShow, 'After Rotation');
            plotVis.visualizeFactorLoadingsBar(k, params.rotationType, params.numVariablesToShow, 'After Rotation');
        end
    end

    % Plot the top tickers explaining each factor
    if isfield(params, 'visualizeTopTickers') && params.visualizeTopTickers
        numTopTickers = params.numTopTickers;  % Number of top tickers to display
        plotVis.plotTopTickersForFactors(k, numTopTickers);
    end
    
    % Compute Factor returns
    estFactorRtns = mktRtns * factorLoadings;
    % Check for NaNs or Infs in estFactorRtns
    if any(isnan(estFactorRtns(:))) || any(isinf(estFactorRtns(:)))
        error('estFactorRtns contains NaN or Inf values after computation.');
    end
    % Display size
    disp(['Size of estFactorRtns: ', num2str(size(estFactorRtns))]);
