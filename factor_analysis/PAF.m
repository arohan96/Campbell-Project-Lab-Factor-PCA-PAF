function [eigenValues, eigenVectors, communalities] = PAF(corrMatrix, ...
...
%% Outputs:
%   eigenValues: Vector of eigenvalues ordered from highest to lowest
%   (positive only)
%   eigenVectors: Matrix where each column represents an eigen vector
%   corresponding to an eigenvalue. Columns are ordered in order of
...
%   communalities: Final communalities amongst factors
    ...
    while max(comm_diff) > tolerance && iteration <= iterations
        ...
        reducedMatrix(1:size(corrMatrix, 1) + 1:end) = u_prev;
        % Eigen decomposition
